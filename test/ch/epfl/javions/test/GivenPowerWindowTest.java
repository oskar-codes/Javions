...
class GivenPowerWindowTest {
    ...
    void powerWindowAdvanceByCanAdvanceOverSeveralBatches() throws IOException {
        ...
        var sampleBytes = Base64.getDecoder().decode(GivenPowerComputerTest.SAMPLES_BIN_BASE64);
        ...
        try (var s = new ByteArrayInputStream(bytes)) {
            ...
            var expected = Arrays.copyOfRange(GivenPowerComputerTest.POWER_SAMPLES, 0, STANDARD_WINDOW_SIZE);
    ...
    void powerWindowIsFullWorks() throws IOException {
        int twoBatchesPlusOneWindowBytes =
                bytesForPowerSamples(BATCH_SIZE * 2 + STANDARD_WINDOW_SIZE);
        byte[] twoBatchesPlusOneWindow = new byte[twoBatchesPlusOneWindowBytes];
    ...
    void powerWindowGetWorksOnGivenSamples() throws IOException {
        try (var sampleStream = GivenPowerComputerTest.getSamplesStream()) {
            var windowSize = 100;
            var w = new PowerWindow(sampleStream, windowSize);
            for (var offset = 0; offset < 100; offset += 1) {
                var expected = Arrays.copyOfRange(GivenPowerComputerTest.POWER_SAMPLES, offset, offset + windowSize);
    ...
    void powerWindowGetWorksAcrossBatches() throws IOException {
        ...
        try (var s = new ByteArrayInputStream(bytes)) {
            ...
            for (int i = 0; i < STANDARD_WINDOW_SIZE; i += 1)
                assertEquals(GivenPowerComputerTest.POWER_SAMPLES[i], w.get(i));
        }
    }
